...
def translate_coords(coords_list, vector)
    coords_list.map { |coords_line|
        coords_line.map { |coords| Point.new(coords.x + vector.x, coords.y + vector.y) }
    }
end

def max_coords(coords_list)
    flattened = coords_list.flatten
    max_x = flattened.map { _1.x }.max
    max_y = flattened.map { _1.y }.max
    [max_x, max_y]
end
...
embiggen_by = Vector.new(126, 2)
min_x, min_y = min_coords(coords_list)
vector = Vector.new(-min_x + embiggen_by.x, 0)
coords_list = translate_coords(coords_list, vector)
max_x, max_y = max_coords(coords_list)
cave = prepare_cave(coords_list, max_x + 1 + embiggen_by.x, max_y + 1 + embiggen_by.y)
sand_point = Point.new(0 - min_x + embiggen_by.x, 0)
